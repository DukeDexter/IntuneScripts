...
Includes fallback logic and defensive paging.
#>
    ...
    while ($next) {
        try {
            ...
            if ($_.Exception.Message -match 'Skip token is null|skiptoken') {
                Write-Warning "Paging halted due to invalid skiptoken. Returning collected items."
                break
            }
            throw
        }
...
Write-Host "Completed. Results saved to $csvPath" -ForegroundColor Green $body = @{
        client_id     = $ClientId
