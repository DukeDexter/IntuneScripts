    ...
    try {
        if ([Environment]::Is64BitOperatingSystem -and -not [Environment]::Is64BitProcess) {
            ...
            if (Test-Path $sysNative) {
                ...
                Write-Log "SysNative PowerShell not found; continuing in current process."
            }
        }
    } catch {
...
function Get-PolicyRulesState {
    ...
    try {
        ...
        if ($present) {
            try {
                # Query via reg.exe to get exact type/value (more reliable when non-REG_SZ)
...
switch ($Mode) {
    ...
    'Report' {
        # PS 5.1 compatibility: precompute the Value instead of inline-if in hashtable
        $safeValue = $null
        if ($null -ne $state.Value) { $safeValue = $state.Value }
        ...
        $resultObj = [pscustomobject]@{
            ...
            Value     = $safeValue
               ...
               $resultObj | ConvertTo-Json -Depth 3 | Write-Output
        exit 0
    }
